...
import { Key, AlertTriangle, Lightbulb, TrendingDown, Target, Clock, Check, X, DollarSign, Trophy, CircleAlert, LineChart } from 'lucide-react';
...
function OverviewTab() {
  ...
  const INSIGHTS = [
    { icon: <Key size={18} />, color: T.green, title: 'Win Pattern Detected', body: 'All 9 won deals had a clearly defined next step set before the call ended.' },
    { icon: <AlertTriangle size={18} />, color: T.red, title: 'Talk Ratio Alert', body: '3 reps exceeded 65% talk time on lost deals. Winning calls averaged 40%.' },
    { icon: <Lightbulb size={18} />, color: T.gold, title: 'Champion Strategy', body: 'Liam and Arjun identified an internal champion within the first 15 minutes.' },
    { icon: <TrendingDown size={18} />, color: T.red, title: 'Pricing Timing Risk', body: 'Sofia presented pricing before establishing ROI on 2 lost deals.' },
    { icon: <Target size={18} />, color: T.blue, title: 'Discovery Depth', body: 'Won deals averaged 9.2/10 on pain discovery. Lost deals averaged 4.1.' },
    { icon: <Clock size={18} />, color: T.gold, title: 'Ideal Call Length', body: 'Sweet spot: 35–51 min. Calls over 60 min had only a 14% win rate.' },
  ];
    ...
    <div className="scenario1-page">
      <header className="scenario1-header">
        <div className="scenario1-header-left">
          ...
          <div className="scenario1-header-logo">
            <div className="scenario1-header-logo-icon" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}><BarChart size={24} /></div>
            <div>
      ...
      <div className="scenario1-body">
        <div className="kpi-grid scenario1-kpi-row">
          <KpiCard label="Revenue Won" value={fmt(revenue)} sub="↑ 18% vs prev period" accent={T.green} icon={<DollarSign size={20} />} />
          <KpiCard label="Win Rate" value={`${winRate}%`} sub="9 of 15 deals closed" accent={T.gold} icon={<Trophy size={20} />} />
          <KpiCard label="Avg Call Score" value={`${avgScore}/100`} sub="AI-assessed quality" accent={T.blue} icon={<Target size={20} />} />
          <KpiCard label="Pipeline at Risk" value={fmt(atRisk)} sub="7 deals lost" accent={T.red} icon={<CircleAlert size={20} />} />
          <KpiCard label="Hours Lost" value={`${lostHrs}h`} sub="on failed deals" accent={T.orange} icon={<Clock size={20} />} />
        </div>
