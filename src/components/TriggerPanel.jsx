...
import { Settings, User, Calendar, Clock, Tag, Lightbulb, Check, X, Loader2, Play, FastForward } from 'lucide-react';
      ...
      <div className={`trigger-body ${collapsed ? 'collapsed' : ''}`}>
        ...
        <div className="form-actions" style={{ marginTop: 18 }}>
          <button className="btn" onClick={handleRun} disabled={isRunning}>
            {isRunning ? <><Loader2 size={16} className="animate-spin" /> Running...</> : <><Play size={16} /> Run Analysis</>}
          </button>
          <button className="btn-secondary" onClick={handleDefaults} disabled={isRunning} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <FastForward size={16} /> Skip — use last week's defaults
          </button>
          {status && (
            <span className={`status-pill ${status.type === 'success' ? 'success' : status.type === 'error' ? 'error' : ''}`} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              {status.type === 'loading' ? <Loader2 size={14} className="animate-spin" /> : status.type === 'success' ? <Check size={14} /> : <X size={14} />} {status.message}
            </span>
          )}
        </div>

        <div className="form-hint" style={{ display: 'flex', gap: 6 }}>
          <Lightbulb size={16} style={{ flexShrink: 0, marginTop: 2 }} />
          <div>All fields optional. Results appear on this dashboard in real-time after ~2 minutes and are also sent via email.</div>
        </div>
      </div>
    </div>
  );
