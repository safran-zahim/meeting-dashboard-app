...
import { Network, Zap, VolumeX, AlertTriangle, Lightbulb, TrendingUp } from 'lucide-react';
...
const keyInsights = [
  { severity: 'critical', icon: <Zap size={16} />, headline: 'Senior leadership holds 62% of speaking time', detail: 'In cross-functional syncs, C-Suite + VP tier speak for 62% of total meeting time. ICs average just 4.2 minutes per session.', tag: 'Voice Dominance' },
  { severity: 'critical', icon: <VolumeX size={16} />, headline: 'QA & Infrastructure teams are effectively silent', detail: 'QA averages 4% speaking time. Infrastructure averages 7%. Both are present in 100% of stand-ups but contribute to under 5% of documented decisions.', tag: 'Disengagement' },
  { severity: 'warning', icon: <AlertTriangle size={16} />, headline: 'Daniel Ortiz leads interruption count at 32 incidents', detail: 'Sales VP interrupts peers 32 times across 11 meetings — predominantly targeting Design and Infrastructure leads. Pattern suggests status-based silencing.', tag: 'Interruptions' },
  { severity: 'warning', icon: <Lightbulb size={16} />, headline: 'Problem-raising concentrated in middle management', detail: 'Team Leads raise 3.4× more problems than solutions. Sr. Leadership proposes 2.1× more solutions than they identify problems — a potential blind-spot dynamic.', tag: 'Problem vs. Solution' },
  { severity: 'positive', icon: <TrendingUp size={16} />, headline: 'Participation equity improving week-over-week', detail: 'Equity score rose 38 → 48 over 6 weeks following the new structured-turn agenda rollout. Still below the healthy 60+ threshold.', tag: 'Trend' },
];
    ...
    <div className={`scenario4-page ${loaded ? 'scenario4-loaded' : ''}`}>
      <header className="scenario4-header">
        <div className="scenario4-header-left">
          ...
          <div>
            <div className="scenario4-header-badge" style={{ display: 'flex', alignItems: 'center', gap: 6 }}><Network size={14} /> Fathom AI · Leadership Intelligence</div>
      ...
      <div className="scenario4-body">
        ...
        <footer className="scenario4-footer">
          <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><Network size={14} /> Leadership Visibility Dashboard · Powered by Fathom AI · {period}</span>
